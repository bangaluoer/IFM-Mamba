...
class FrequencyGate(nn.Module):
    ...
    def forward(self, x):
        ...
        x_fft = x_fft * weight.view(1, C, 1, 1)
        x_spatial = torch.fft.irfft2(x_fft, s=(H, W), norm='backward')
...
class NAFBlock(nn.Module):
    ...
    def forward(self, x, x_size, params=None):
        ...
        inp = x.transpose(1, 2).view(B, C, H, W)
...
class ASSM(nn.Module):
    def __init__(self, dim, d_state, input_resolution, num_tokens=128, inner_rank=128, mlp_ratio=2.):
        super().__init__()
        self.dim = dim
...
class BasicBlock(nn.Module):
    """ A basic ASSB for one stage. (Modified for Hybrid Architecture) """
    ...
    def __init__(self,
        ...
        for i in range(depth):
            if i < depth // 2:
                self.layers.append(NAFBlock(c=dim))
                if idx == 0 and i == 0:
                    print(f"✅ Hybrid Surgery: Stage {idx} initialized with NAFBlocks for the first {depth//2} layers.")
            else:
                self.layers.append(
                    AttentiveLayer(
                        dim=dim,
...
class MambaIRv2Light(nn.Module):
    def __init__(self,
        ...
        self.fft_gate = FrequencyGate(embed_dim)
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            feat_main = self.forward_features(x_first, params)
            
            feat_freq = self.fft_gate(feat_main)
            
            res = self.conv_after_body(feat_main + feat_freq) + x_first
